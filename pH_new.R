...
#save dataset for DOE upload
ph = ph[,-2]
colnames(ph)[1] <- "SampleID"
colnames(ph)[2] = "Heat_Flux"
#write.csv(ph, "~/Library/CloudStorage/Box-Box/WhitmanLab/Projects/Repeated_Fire/manuscript_test/data_DOE/pH_of_all_samples.csv",row.names = FALSE)
...
#save dataset for DOE upload
merge.T.ph = merge.T.ph[,-c(1,6,7,8,9,11,12,13,15)]
colnames(merge.T.ph)[2] = "Heat_Flux"
colnames(merge.T.ph)[3] = "Depth"
colnames(merge.T.ph)[5] <- "Peak_T_°C"
colnames(merge.T.ph)[6] <- "Degree_Hours_(°C_x_hrs)"
#write.csv(merge.T.ph, "~/Library/CloudStorage/Box-Box/WhitmanLab/Projects/Repeated_Fire/manuscript_test/data_DOE/pH_vs_peakT_and_degree_hours_only_for_heated_samples.csv",row.names = FALSE)
